...
test('PWA conserva Créditos y Caja dentro del caché vigente de v3.3.1', () => {
    const sw = read('public/service-worker.js');
    const index = read('public/index.html');
    assert.match(sw, /v3\.3\.1-kitchen-traceability/);
    assert.match(index, /accounts\.js\?v=3\.3\.1-kitchen-traceability/);
    assert.match(index, /cash\.js\?v=3\.3\.1-kitchen-traceability/);
});
