...
test('PWA conserva Créditos y Caja dentro del caché vigente de v3.4.2', () => {
    const sw = read('public/service-worker.js');
    const index = read('public/index.html');
    assert.match(sw, /v3\.4\.2-printer-settings/);
    assert.match(index, /accounts\.js\?v=3\.4\.2-printer-settings/);
    assert.match(index, /cash\.js\?v=3\.4\.2-printer-settings/);
});
